...
5. To use the application on the web, you will need to provide a `Google Maps API Key` in order to use the `Location` and `MapView` integrations provided by `Expo`. We explicitly make it easy to do so, by utilizing a `.env` file located in the root of this repository. This file will get loaded via `dotenv` and provided by our `webpack config` to the application. This environment variable will get interpolated in the right places, in the `index.html` files located in the `web` folders of the corresponding packages. Just use the file `example.env` provided with this repository, rename the file to simply `.env` and input your api key. You can generate a new API Key, following this [guide](https://developers.google.com/maps/documentation/javascript/get-api-key).
